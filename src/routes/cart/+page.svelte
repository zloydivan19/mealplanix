...
<div class="pg">
  ...
  <header class="hd">
    ...
    <div class="hd-meta">
      <div class="prog-wrap">
        ...
        <span class="prog-label">{checkedCount} / {totalItems}</span>
      </div>
      <div class="hd-actions">
        {#if checkedCount > 0}
        <button class="btn-reset" onclick={resetChecked} aria-label="Сбросить все отметки">
          <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
            <path d="M1.5 1.5l9 9M10.5 1.5l-9 9" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/>
          </svg>
          Сбросить
        </button>
        {/if}
        <button class="btn-export" onclick={exportCart}>
          <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
            <rect x="1.5" y="4" width="7" height="8.5" rx="1.5" stroke="currentColor" stroke-width="1.4"/>
            <path d="M4.5 4V2.5A1 1 0 015.5 1.5h5A1 1 0 0111.5 2.5v8a1 1 0 01-1 1H9" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/>
          </svg>
          Копировать
        </button>
      </div>
  ...
  <div class="bd">
    ...
    <div class="empty">
      <div class="empty-icon">
        <svg width="36" height="36" viewBox="0 0 36 36" fill="none">
          <path d="M6 8h3l4 14h12l3-10H11" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
    ...
    {@const groupDone = group.items.filter(i => checked.has(i.name)).length}
      ...
      <div class="cat-hd">
        <div class="cat-icon cat-icon--{group.category}">
          {#if group.category === 'meat'}
            <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><path d="M9.5 3C8 1.5 5.5 2 4 3.5L2 5.5l2 2 1-1 3.5 3.5-1 1L9 13l2.5-2.5C13 9 12.5 6 11 4.5" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/></svg>
          {:else if group.category === 'dairy'}
            <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><path d="M4.5 2h4l1.5 2.5H3L4.5 2z" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/><path d="M3 4.5L2.5 11a.5.5 0 00.5.5h7a.5.5 0 00.5-.5L10 4.5H3z" stroke="currentColor" stroke-width="1.3"/><path d="M5 7.5h3" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/></svg>
          {:else if group.category === 'grain'}
            <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><path d="M6.5 11.5V5" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/><ellipse cx="6.5" cy="4" rx="2.5" ry="1.5" stroke="currentColor" stroke-width="1.3"/><path d="M4 7c-1.5 0-2.5-1-2.5-2.5" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/><path d="M9 7c1.5 0 2.5-1 2.5-2.5" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/></svg>
          {:else if group.category === 'vegetable'}
            <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><path d="M6.5 11.5V6" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/><path d="M6.5 6C5 6 2 4 3 1.5c2.5 0 4 2 3.5 4.5z" stroke="currentColor" stroke-width="1.3" fill="currentColor" fill-opacity="0.15"/><path d="M6.5 8c1.5 0 4.5-2 3.5-4.5C7.5 3.5 6 5.5 6.5 8z" stroke="currentColor" stroke-width="1.3" fill="currentColor" fill-opacity="0.15"/></svg>
          {:else if group.category === 'fruit'}
            <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><circle cx="6.5" cy="8" r="3.5" stroke="currentColor" stroke-width="1.3"/><path d="M6.5 4.5V3M5.5 3c0-1 2-1 2 0" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/></svg>
          {:else if group.category === 'condiment'}
            <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><path d="M5 2h3v2.5l1.5 1.5H3.5L5 4.5V2z" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/><path d="M3.5 6l-.5 5a.5.5 0 00.5.5h5.5a.5.5 0 00.5-.5L9 6" stroke="currentColor" stroke-width="1.3"/></svg>
          {:else}
            <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><rect x="1.5" y="1.5" width="10" height="10" rx="2" stroke="currentColor" stroke-width="1.3"/><path d="M4 6.5h5M4 4.5h3" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/></svg>
          {/if}
        </div>
        <span class="cat-name">{SHOPPING_CATEGORY_LABELS[group.category]}</span>
        <div class="cat-dots" aria-hidden="true">
          {#each group.items as _, dotIdx}
            ...
            <div class="item-cb" class:item-cb--on={isChecked}>
              {#if isChecked}
                <svg width="9" height="7" viewBox="0 0 9 7" fill="none">
                  <path d="M1 3.5l2 2.5 5-5" stroke="white" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
              {/if}
            </div>
                ...
                {#if fridgeEntry && item.unit && fridgeEntry.unit === item.unit}
                  <span class="item-fridge">
                    <svg width="9" height="10" viewBox="0 0 9 10" fill="none">
                      <rect x="1" y="0.5" width="7" height="9" rx="1.2" stroke="currentColor" stroke-width="1.2"/>
                      <path d="M1 4h7" stroke="currentColor" stroke-width="1.2"/>
                      <path d="M4.5 2v1.5" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/>
                    </svg>
                    −{fridgeEntry.qty} {fridgeEntry.unit}
                  </span>
                {/if}
...
<div class="toast" role="status" aria-live="polite">
  <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
    <path d="M2 7l3.5 4L12 3" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
  </svg>
  Скопировано в буфер
</div>
{/if}
...
.btn-export:hover { background: var(--color-green-dark); }
...
.cat-icon--meat      { background: rgba(224,123,57,0.13); color: var(--cat-meat-text); }
.cat-icon--dairy     { background: rgba(196,148,58,0.13); color: var(--cat-dairy-text); }
.cat-icon--grain     { background: rgba(180,150,80,0.13); color: var(--cat-grain-text); }
.cat-icon--vegetable { background: var(--color-green-tint); color: var(--cat-vegetable-text); }
.cat-icon--fruit     { background: rgba(190,60,110,0.11); color: var(--cat-fruit-text); }
.cat-icon--condiment { background: rgba(74,127,193,0.11); color: var(--cat-condiment-text); }
.cat-icon--other     { background: rgba(107,117,104,0.10); color: var(--cat-other-text); }
...
.cat-dot--on { background: var(--color-green-primary); }
.cat-count {
  font-size: 11px;
  font-weight: 600;
  color: var(--color-text-muted);
...
.item-row:hover { background: var(--color-green-tint); }
.item-row--done { background: var(--color-bg-page); }
.item-row--done:hover { background: var(--color-bg-page); }
...
.item-qty--done { text-decoration: line-through; }
...
.item-need--zero {
  ...
  background: rgba(26,158,94,0.10);
}
...
.price-input:focus { border-color: var(--color-green-primary); }
.price-cur {
  font-size: 12px;
  color: var(--color-text-muted);
}
...
.summary-cell--accent .summary-lbl { color: rgba(255,255,255,0.65); }
.summary-val {
  font-size: 17px;
  font-weight: 700;
  color: var(--color-text-primary);
...
.summary-cell--accent .summary-val { color: #fff; }
.summary-div {
  width: 1px;
  background: var(--color-border);
  margin: 8px 0;
...
@media (prefers-reduced-motion: reduce) {
  .prog-fill, .item-cb, .item-row, .cat-dot { transition: none !important; }
