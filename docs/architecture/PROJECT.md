...
**Хостинг:** Netlify — https://mealplanix.netlify.app  
...
│   │   ├── SmartReplaceModal.svelte # Умная замена блюда по КБЖУ
│   │   ├── FridgeSelectModal.svelte # Выбор продуктов из холодильника для генерации
│   │   ├── SaveTemplateModal.svelte # Сохранить неделю как шаблон меню
│   │   └── ApplyTemplateModal.svelte # Применить шаблон меню (заменить / дополнить)
│   ├── data/
...
| `menu_plans` | Планы питания: одна строка на слот блюда (persona_id + week_label + day_index + meal_key) |
| `menu_templates` | Шаблоны меню: сохранённые недели для повторного применения |
| `cart_state` | Состояние корзины: отметки куплено + цены |
| `food_catalog` | Каталог ~83 блюд с КБЖУ, ингредиентами и фото |
| `custom_dishes` | Пользовательские блюда (JSONB), поддерживают поле photo |
| `household_fridge` | Содержимое холодильника: одна строка на продукт (qty, unit, expires_at) |
...
3. **Масштабирование порции:**
   ...
   - Зажимается в `[portion_min_g, min(portion_max_g, CATEGORY_MAX_G)]`
   - Лимиты по категории: `salad ≤ 300г`, `snack ≤ 400г`
...
| `migration-menu-templates.sql` | Создание таблицы menu_templates + RLS (V5-096) |
| `migration-dish-photos.sql` | Добавление колонки photo в food_catalog + начальные URL |
| `migration-dish-photos-2.sql` | Дополнительные фото для food_catalog |
...
| Меню (генерация, шаблоны, отображение) | ✔️ полностью готово |
| Блюда (каталог, кастомные, FatSecret, фото) | ✔️ готово |
| Корзина (агрегация, цены, чекбоксы, холодильник) | ✔️ готово |
| Холодильник (CRUD, вычитание, генерация по остаткам) | ✔️ готово |
| Семья и персоны | ✔️ готово |
| Настройки | ✔️ готово |

**Следующие задачи (Phase 3):**
- V5-100: Мобильный режим покупок (shopping mode)
- V5-098: Страница /stats — КБЖУ-график vs цель персоны
- V5-083: Загрузка фото блюд в Supabase Storage
