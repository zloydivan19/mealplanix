<script lang="ts">
  ...
  async function save() {
    ...
    if (matchKcal && !kbju) { errorMsg = 'Заполни возраст, вес и рост'; return; }
    if (matchKcal && !ratioValid) { errorMsg = 'Сумма % по приёмам пищи должна быть 100'; return; }
...
<div class="flex flex-col min-h-screen" style="background: var(--color-bg-page);">
  ...
  <!-- Топбар 56px -->
  <div class="sticky top-0 flex items-center gap-3 px-4" style="height: 56px; background: var(--color-bg-card); border-bottom: 1px solid var(--color-border); z-index: 10;">
    <a
      href="/"
      class="w-8 h-8 flex items-center justify-center rounded-lg transition-colors cursor-pointer shrink-0"
      style="color: var(--color-text-muted);"
      onmouseenter={e => { const el = e.currentTarget as HTMLElement; el.style.background = 'var(--color-bg-page)'; el.style.color = 'var(--color-text-primary)'; }}
    ...
    <h1 class="font-semibold" style="font-size: 15px; color: var(--color-text-primary); letter-spacing: -0.01em;">Настройки</h1>
  </div>

<div class="max-w-lg mx-auto w-full px-4 py-6">

  {#if persona}
    <div class="flex flex-col gap-4">

      <!-- ── РАЗДЕЛ: Профиль ──────────────────────────────── -->
      <div style="background: var(--color-bg-card); border-radius: var(--radius-lg); box-shadow: var(--shadow-card); border: 1px solid var(--color-border); padding: 20px;">
        <h2 class="font-semibold mb-1" style="font-size: 16px; color: var(--color-text-primary);">Профиль</h2>
        <p class="text-sm mb-4" style="color: var(--color-text-muted);">Личные данные для расчёта нормы</p>

        <div class="flex flex-col gap-4">

          <!-- Имя -->
          <div>
            <label for="name" class="block text-xs font-semibold mb-1.5" style="color: var(--color-text-primary);">Имя</label>
            <input id="name" type="text" bind:value={name} placeholder="Катя" class="brand-input" />
          </div>
          ...
          <!-- Активность -->
          <div>
            <label for="activity" class="block text-xs font-semibold mb-1.5" style="color: var(--color-text-primary);">Уровень активности</label>
          ...
          <!-- Формула расчёта -->
          <div>
            <label for="formula" class="block text-xs font-semibold mb-1.5" style="color: var(--color-text-primary);">Формула расчёта калорий</label>
      ...
      <div style="background: var(--color-bg-card); border-radius: var(--radius-lg); box-shadow: var(--shadow-card); border: 1px solid var(--color-border); padding: 20px;">
        <h2 class="font-semibold mb-1" style="font-size: 16px; color: var(--color-text-primary);">Генерация меню</h2>
        <p class="text-sm mb-4" style="color: var(--color-text-muted);">Как подбирать блюда при автоматическом составлении плана</p>

        <!-- Тумблер match_kcal -->
        <div
          class="mb-4"
          style="border-radius: var(--radius-lg); border: 1px solid {matchKcal ? 'var(--color-green-tint-border)' : 'var(--color-border)'}; overflow: hidden; transition: border-color var(--transition-fast);"
        >
          <button
            type="button"
            onclick={() => matchKcal = !matchKcal}
            class="w-full flex items-center gap-3 px-4 py-3"
            style="background: {matchKcal ? 'var(--color-green-tint)' : 'var(--color-bg-page)'}; border: none; cursor: pointer; transition: background var(--transition-fast);"
          >
            <div
              class="relative w-10 h-6 rounded-full shrink-0"
              style="background: {matchKcal ? 'var(--color-green-primary)' : 'var(--color-border)'}; transition: background var(--transition-fast);"
            >
              <span
                class="absolute top-1 w-4 h-4 rounded-full shadow block"
                style="background: var(--color-text-inverse); transform: translateX({matchKcal ? '20px' : '4px'}); transition: transform var(--transition-fast);"
              ></span>
            </div>
            <span class="text-sm font-semibold text-left" style="color: {matchKcal ? 'var(--color-green-primary)' : 'var(--color-text-primary)'};">
        ...
        {#if matchKcal}
          {#if kbju}
            <div class="mb-4" style="background: var(--color-green-tint); border-radius: var(--radius-lg); padding: 16px; border: 1px solid var(--color-green-tint-border);">
              <p class="text-xs font-semibold uppercase mb-3" style="color: var(--color-green-primary);">Твоя суточная норма</p>
          ...
          {:else}
            <div class="mb-4 text-center py-4" style="background: var(--color-bg-page); border-radius: var(--radius-lg); border: 1px solid var(--color-border);">
          ...
          <div>
            <p class="text-xs font-semibold mb-3" style="color: var(--color-text-primary);">Распределение калорий по приёмам пищи</p>
            <div class="grid grid-cols-3 gap-3">
              {#each [{ id: 'bf', label: 'Завтрак', bind: bf }, { id: 'ln', label: 'Обед', bind: ln }, { id: 'dn', label: 'Ужин', bind: dn }] as item}
                <div>
                  <label for={item.id} class="block text-xs mb-1.5 text-center" style="color: var(--color-text-muted);">{item.label}</label>
                  <div class="relative">
                    {#if item.id === 'bf'}
        ...
        <div class="flex items-start gap-3">
          <button
            ...
            class="relative shrink-0 mt-0.5 w-10 h-6 rounded-full transition-colors cursor-pointer"
            style="background: {carryDinner ? 'var(--color-green-primary)' : 'var(--color-border)'}; border: none; padding: 0;"
          >
            <span
              class="absolute top-1 w-4 h-4 rounded-full shadow transition-transform block"
          ...
          <div>
            ...
            <p class="text-xs mt-0.5" style="color: var(--color-text-muted); line-height: 1.5;">
              Готовишь несколько порций сразу — одна идёт на ужин, другая на завтра в обед.
              При генерации меню обед каждого дня будет совпадать с ужином предыдущего.
            </p>
          </div>
        </div>
      ...
      <div class="flex items-center gap-3">
        ...
        {#if saved}
          <p class="text-sm font-semibold flex items-center gap-1.5" style="color: var(--color-success);">
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none" style="flex-shrink:0;"><path d="M2 7l3.5 3.5 6.5-6" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>
            Сохранено
          </p>
        {/if}
      </div>
